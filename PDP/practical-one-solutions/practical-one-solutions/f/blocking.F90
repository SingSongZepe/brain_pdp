...
contains
	...
	subroutine run_solver()
		...
		do k=0, MAX_ITERATIONS      
			if (myrank .gt. 0) then
				call MPI_Sendrecv(u_k(1,1), ny, MPI_DOUBLE, myrank-1, 0, u_k(1,0), ny, MPI_DOUBLE, &
					myrank-1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE, ierr)
			end if
			if (myrank .lt. size-1) then
				call MPI_Sendrecv(u_k(1,local_nx), ny, MPI_DOUBLE, myrank+1, 0, u_k(1,local_nx+1), ny, &
